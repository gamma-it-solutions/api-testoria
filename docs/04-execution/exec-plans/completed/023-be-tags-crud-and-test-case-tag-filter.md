...
- [x] ~~Create Alembic migration adding a unique index on `lower(tag.name)`~~ — Skipped: normalizing names to lowercase on write makes existing unique(name) index sufficient
- [x] ~~Apply migration locally~~ — No migration needed
...
- [x] ~~`docs/06-generated/db-schema.md`~~ — No schema change (normalization is in application layer)
...
- [x] ~~Migration applies cleanly and is reversible~~ — No migration needed (normalization on write)
- [x] Docs updated
